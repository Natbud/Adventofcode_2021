...
for line in file_list:
    ...
    if y1 == y2:
        ...
        while not d > (int(max(x1, x2))):
            ...
            d += 1
            # print("Line_of_points:", line_of_points)
    # NOTE - Diagonals are accessed from the top left corner and increment right
    # and down.

    # check for diagonallines:
    if (max(y2, y1) - (min(y1, y2)) == (max(x2, x1) - (min(x1, x2)))):
        print("is a diagonal")
        if (y1 > y2 and x1 < x2):
            print("Is +ve gradient case 1")
            d = (int(min(x1, x2)))
            v = (int(max(y1, y2)))
            line_of_points = []
            while not d > (int(max(x1, x2))):
                point = (list(map(int, (d, v))))
                line_of_points.append(point)
                np_all_positions[point[1]][point[0]] += 1
                d += 1
                v -= 1
            print("line_of_points case 1:", line_of_points)

        if (y1 < y2 and x1 > x2):
            print("Is +ve gradient case 2")
            d = (int(max(x1, x2)))
            v = (int(min(y1, y2)))
            line_of_points = []
            while not d < (int(min(x1, x2))):
                point = (list(map(int, (d, v))))
                line_of_points.append(point)
                np_all_positions[point[1]][point[0]] += 1
                d -= 1
                v += 1
            print("line_of_points case 2:", line_of_points)


        if y1 < y2 and x1 < x2: # this is diag is -ve gradient
            print("is -ve gradient case 3")
            d = (int(min(x1, x2)))
            v = (int(min(y1, y2)))
            line_of_points = []
            while not d > (int(max(x1, x2))):
